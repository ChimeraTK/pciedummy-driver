...
static int __init mtcaDummy_init_module(void) {
  ...
  dbg_print("%s\n", "MODULE INIT");
  status = alloc_chrdev_region(&dev, mtcaDummyMinorNr, MTCADUMMY_NR_DEVS,
                               MTCADUMMY_NAME);
  if (status) {
  ...
  for (i = 0; i < MTCADUMMY_NR_DEVS; i++) {
    ...
    dummyPrivateData[i].systemBar =
        kmalloc(MTCADUMMY_N_REGISTERS * sizeof(u32), GFP_KERNEL);
    if (dummyPrivateData[i].systemBar == NULL) {
      goto err_allocate_systemBar;
    }
    memset(dummyPrivateData[i].systemBar, 0,
           MTCADUMMY_N_REGISTERS * sizeof(u32));
...
err_allocate_systemBar:
   ...
   */
  for (j = 0; j < i; j++) {
    device_destroy(mtcaDummyClass,
                   MKDEV(mtcaDummyMajorNr, mtcaDummyMinorNr + j));
...
err_class_create:
  unregister_chrdev_region(MKDEV(mtcaDummyMajorNr, mtcaDummyMinorNr),
                           MTCADUMMY_NR_DEVS);
...
static void __exit mtcaDummy_cleanup_module(void) {
  ...
  for (i = 0; i < MTCADUMMY_NR_DEVS; i++) {
    device_destroy(mtcaDummyClass,
                   MKDEV(mtcaDummyMajorNr, mtcaDummyMinorNr + i));
    cdev_del(&dummyPrivateData[i].cdev);
    mutex_destroy(&dummyPrivateData[i].devMutex);
    kfree(dummyPrivateData[i].systemBar);
...
static ssize_t mtcaDummy_read(struct file *filp, char __user *buf, size_t count,
                              loff_t *f_pos) {
  mtcaDummyData *privData;
  ...
  if (readReqInfo.mode_rw == RW_INFO) {
    ...
    if (mtcadummy_performPreReadAction(readReqInfo.offset_rw,
      ...
      dbg_print("Simulating read access to bad register at offset %d, %s\n",
                readReqInfo.offset_rw, "intentinally causing an I/O error");
      mutex_unlock(&privData->devMutex);
      return -EIO;
    }
    ...
    if (copy_to_user(buf, (void *)(privData->dmaBar + (dma_offset / 4)),
                     dma_size)) {
...
static ssize_t mtcaDummy_write(struct file *filp, const char __user *buf,
  ...
  if (writeReqInfo.mode_rw == RW_D32) {
    ...
    if (mtcadummy_performActionOnWrite(
            writeReqInfo.offset_rw, writeReqInfo.barx_rw, privData->slotNr)) {
      dbg_print("Simulating write access to bad register at offset %d, %s.\n",
                writeReqInfo.offset_rw, "intentinally causing an I/O error");
      mutex_unlock(&privData->devMutex);
      return -EIO;
    }
...
static long mtcaDummy_ioctl(struct file *filp, unsigned int cmd,
                            unsigned long arg) {
  ...
  case PCIEDEV_PHYSICAL_SLOT:
    if (copy_from_user(&dataStruct, (device_ioctrl_data *)arg,
                       sizeof(device_ioctrl_data))) {
      mutex_unlock(&privateData->devMutex);
      return -EFAULT;
    }
    dataStruct.data = privateData->slotNr;
    if (copy_to_user((device_ioctrl_data *)arg, &dataStruct,
                     sizeof(device_ioctrl_data))) {
      returnValue = -EFAULT;
  ...
  case PCIEDEV_READ_DMA:
    ...
    // (variable 'pattern')
    if (copy_from_user(&dmaStruct, (device_ioctrl_dma *)arg,
                       sizeof(device_ioctrl_dma))) {
      mutex_unlock(&privateData->devMutex);
      return -EFAULT;
    }
    ...
    if (copy_to_user((u32 *)arg,
                     privateData->dmaBar + (dmaStruct.dma_offset / 4),
                     dmaStruct.dma_size)) {
      returnValue = -EFAULT;
      // mutex will be unlocked directly after the switch
    }
 ...
 */
static long llrfDummy_ioctl(struct file *filp, unsigned int cmd,
                            unsigned long arg) {
  mtcaDummyData *privateData;
  int status = 0;
  device_ioctrl_data dataStruct;
 ...
 * old drivers.
 */
static long pcieuniDummy_ioctl(struct file *filp, unsigned int cmd,
                               unsigned long arg) {
  mtcaDummyData *privateData;
  int status = 0;
  device_ioctrl_data dataStruct;
